...
-- by each window.
local BlurEffect = nil -- kept for backwards-compatible unload cleanup
...
local IMAGE_ASSET_FOLDER = ASSETS_FOLDER .. "/Images"
...
local function EnsureImageFolder()
	...
	local ok = pcall(function()
		if not fn_isfolder(IMAGE_ASSET_FOLDER) then fn_makefolder(IMAGE_ASSET_FOLDER) end
	end)
	return ok
end

local function ImageAssetPath(source)
	local safe = tostring(source):gsub("[^%w_%-]", "_")
	if #safe > 72 then safe = safe:sub(1, 72) end
	local extension = tostring(source):lower():match("%.([%a%d]+)%?") or tostring(source):lower():match("%.([%a%d]+)$") or "png"
	if extension ~= "png" and extension ~= "jpg" and extension ~= "jpeg" and extension ~= "gif" and extension ~= "webp" then
		extension = "png"
	end
	return IMAGE_ASSET_FOLDER .. "/" .. safe .. "." .. extension
end

local function ResolveImageSource(source, onReady)
	...
	if not source:match("^https?://") then return ResolveIcon(source) end
	if ImageAssetCache[source] then return ImageAssetCache[source] end

	if onReady then
		task.spawn(function()
			local result = source
			if fn_customasset and fn_writefile and EnsureImageFolder() then
				local path = ImageAssetPath(source)
				local exists = false
...
local function SetImageSource(image, source)
	if not image then return end
	local initial = ResolveImageSource(source, function(asset)
...
function NullUI:CreateWindow(opts)
	...
	if opts.SidebarPanel == true or type(opts.SidebarPanel) == "table" then
		local sidebarInfo = type(opts.SidebarPanel) == "table" and opts.SidebarPanel or opts
		local panelHeight = 112
		sidebarOffset = panelHeight + 10

		sidebarPanel = Instance.new("Frame")
		sidebarPanel.Name = "SidebarPanel"
		sidebarPanel.BackgroundColor3 = Color3.fromRGB(8, 8, 8)
		sidebarPanel.BackgroundTransparency = 0
		sidebarPanel.BorderSizePixel = 0
		sidebarPanel.ClipsDescendants = true
		sidebarPanel.Position = UDim2.fromOffset(margin, 58)
		sidebarPanel.Size = UDim2.fromOffset(130, panelHeight)
		sidebarPanel.ZIndex = Z.Content + 1
		sidebarPanel.Parent = main
		Corner(sidebarPanel, 10)
		Stroke(sidebarPanel, Color3.new(1, 1, 1), 1, 0.88)

		local sidebarImage = Instance.new("ImageLabel")
		sidebarImage.Name = "Image"
		sidebarImage.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
		sidebarImage.BackgroundTransparency = 0
		sidebarImage.BorderSizePixel = 0
		sidebarImage.Size = UDim2.fromScale(1, 1)
		...
		sidebarFade.Name = "BlackFade"
		sidebarFade.BackgroundColor3 = Color3.new(0, 0, 0)
		sidebarFade.BackgroundTransparency = 0.22
		sidebarFade.BorderSizePixel = 0
		sidebarFade.AnchorPoint = Vector2.new(0, 1)
		sidebarFade.Position = UDim2.fromScale(0, 1)
		...
		fadeGradient.Parent = sidebarFade

		local sidebarTitle = Instance.new("TextLabel")
		sidebarTitle.Name = "Title"
		sidebarTitle.BackgroundTransparency = 1
		sidebarTitle.FontFace = NullUI.Theme.Font
		sidebarTitle.Text = sidebarInfo.SidebarTitle or sidebarInfo.Title or "Hello"
		sidebarTitle.TextColor3 = Color3.new(1, 1, 1)
		sidebarTitle.TextSize = 13
		sidebarTitle.TextXAlignment = Enum.TextXAlignment.Left
		sidebarTitle.TextTruncate = Enum.TextTruncate.AtEnd
		sidebarTitle.Position = UDim2.new(0, 8, 1, -33)
		sidebarTitle.Size = UDim2.new(1, -16, 0, 17)
		sidebarTitle.ZIndex = Z.Content + 3
		sidebarTitle.Parent = sidebarPanel

		local sidebarDescription = Instance.new("TextLabel")
		sidebarDescription.Name = "Description"
		sidebarDescription.BackgroundTransparency = 1
		sidebarDescription.FontFace = NullUI.Theme.FontRegular
		sidebarDescription.Text = sidebarInfo.SidebarDescription or sidebarInfo.Description or "This is example only"
		sidebarDescription.TextColor3 = Color3.fromRGB(220, 220, 220)
		sidebarDescription.TextSize = 10
		sidebarDescription.TextXAlignment = Enum.TextXAlignment.Left
		...
		local viewerInset = GuiService:GetGuiInset()
		viewerBackdrop.Position = UDim2.fromOffset(0, -viewerInset.Y)
		viewerBackdrop.Size = UDim2.new(1, 0, 1, viewerInset.Y)
		...
		viewerCard.Size = UDim2.fromOffset(552, 340)
		...
		viewerImage.Position = UDim2.new(0.5, 0, 0, 10)
		viewerImage.Size = UDim2.fromOffset(512, 288)
		...
		viewerTitle.Position = UDim2.new(0.5, 0, 1, -37)
		...
		viewerDescription.Position = UDim2.new(0.5, 0, 1, -17)
		...
		local function openViewer()
			...
			Tween(viewerBackdrop, { BackgroundTransparency = 0.18 }, 0.34, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
	...
	tabBar.Position = UDim2.fromOffset(margin, tabTop)
	tabBar.Size = UDim2.new(0, 130, 1, -(tabTop + margin))
...
function Tab:AddParagraph(opts)
	...
	if opts.Image and opts.Image ~= "" then
		imageHolder = Instance.new("Frame")
		imageHolder.Name = "ParagraphImage"
		imageHolder.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
		imageHolder.BackgroundTransparency = 0
		imageHolder.BorderSizePixel = 0
		imageHolder.Size = UDim2.new(1, 0, 0, tonumber(opts.ImageHeight) or 92)
		imageHolder.LayoutOrder = 1
		imageHolder.ClipsDescendants = true
		imageHolder.ZIndex = Z.Content + 1
		imageHolder.Parent = card
		Corner(imageHolder, NullUI.Theme.CornerRadiusSm)

		local image = Instance.new("ImageLabel")
		image.Name = "Image"
		image.BackgroundTransparency = 1
		image.Size = UDim2.fromScale(1, 1)
		image.ScaleType = opts.ScaleType or Enum.ScaleType.Crop
		image.ZIndex = Z.Content + 2
		image.Parent = imageHolder
		Corner(image, NullUI.Theme.CornerRadiusSm)
		SetImageSource(image, opts.Image)
	end
	...
	if opts.Title then
		...
		titleRow.LayoutOrder = imageHolder and 2 or 1
	...
	local description = opts.Description or opts.Text or ""
	local textLabel = Instance.new("TextLabel")
	...
	textLabel.LayoutOrder = titleLabel and (imageHolder and 3 or 2) or (imageHolder and 2 or 1)
	textLabel.ZIndex = Z.Content + 1
	textLabel.Parent = card

	return {
		Instance = card,
		Set = function(_, v) textLabel.Text = tostring(v or "") end,
		Get = function() return textLabel.Text end,
		SetTitle = function(_, v) if titleLabel then titleLabel.Text = tostring(v or "") end end,
		SetDescription = function(_, v) textLabel.Text = tostring(v or "") end,
...
function Tab:AddHoldButton(opts)
	opts = opts or {}
	local hasDesc = opts.Description and opts.Description ~= ""
	local height = hasDesc and 68 or 56
	local card = BaseCard(self._page, height)
	local textX = AddLeadingIcon(card, opts.Icon, height)
	AddTitleDesc(card, textX, 42, opts.Text or opts.Title or "Hold Button", opts.Description, height - 8)
	self._window:_RegisterSearchable(self, opts.Text or opts.Title or "Hold Button", card)

	local progressTrack = Instance.new("Frame")
	progressTrack.Name = "HoldProgress"
	...
	progressTrack.ZIndex = Z.Content + 1
	progressTrack.Parent = card
	Corner(progressTrack, 2)

	local progressFill = Instance.new("Frame")
	progressFill.Name = "Fill"
	progressFill.BackgroundColor3 = NullUI.Theme.Accent
	progressFill.BackgroundTransparency = 0
	progressFill.BorderSizePixel = 0
	progressFill.Size = UDim2.fromScale(0, 1)
	progressFill.ZIndex = Z.Content + 2
	...
	local holdTime = math.max(tonumber(opts.HoldTime or opts.Duration) or 1.5, 0.1)
	local held = false
	...
	local function cancel()
		...
		completed = false
		token += 1
		Tween(progressFill, { Size = UDim2.fromScale(0, 1) }, 0.18)
	...
	local function begin()
		...
		if opts.OnStart then task.spawn(opts.OnStart) end
		task.spawn(function()
			local elapsed = 0
			while held and token == currentToken do
				elapsed += RunService.Heartbeat:Wait()
				local alpha = math.clamp(elapsed / holdTime, 0, 1)
				progressFill.Size = UDim2.fromScale(alpha, 1)
				if alpha >= 1 then
					held = false
					completed = true
					if opts.Callback then task.spawn(opts.Callback) end
					...
					task.delay(0.18, function()
						if progressFill.Parent and completed then
							Tween(progressFill, { Size = UDim2.fromScale(0, 1) }, 0.2)
						end
					end)
					break
				end
	...
	click.InputEnded:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			if not completed then cancel() end
		end
	end)
	click.MouseEnter:Connect(function()
		Tween(card, { BackgroundTransparency = 0.9 }, 0.15)
	...
	return {
		...
		Begin = begin,
		Cancel = cancel,
		GetProgress = function() return progressFill.Size.X.Scale end,
		Destroy = function()
			held = false
			token += 1
			card:Destroy()
...
function Tab:AddCustom(instance, opts)
	opts = opts or {}
	if typeof(instance) ~= "Instance" then
		return nil
	end
	local holder = Instance.new("Frame")
	holder.Name = opts.Name or "CustomContent"
	holder.BackgroundTransparency = 1
	holder.BorderSizePixel = 0
	holder.Size = UDim2.new(1, 0, 0, tonumber(opts.Height) or 44)
	holder.AutomaticSize = opts.AutomaticSize and Enum.AutomaticSize.Y or Enum.AutomaticSize.None
	holder.LayoutOrder = opts.LayoutOrder or 1
	holder.ZIndex = Z.Content
	holder.Parent = self._page
	...
	return {
		Instance = holder,
		Content = instance,
		SetHeight = function(_, height) holder.Size = UDim2.new(1, 0, 0, tonumber(height) or 44) end,
		Destroy = function() holder:Destroy() end,
	}
end
