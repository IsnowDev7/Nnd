...
function Library:CreateWindow(WindowInfo)
    ...
    -- Reserve space for the User Panel inside the active tab content area.
    local UserPanelHeight = WindowInfo.AddUserPanel and 80 or 0
    ...
    do
        ...
        if false and WindowInfo.AddUserPanel then
            UserPanelHeight = 80
        ...
        --// USER PANEL \\--
        -- This panel belongs to the tab content column. Top-level tab canvases
        -- are offset by UserPanelHeight, so existing groupboxes move below it.
        if WindowInfo.AddUserPanel then
            ...
            if WindowInfo.Username then
                New("TextLabel", {
                    BackgroundTransparency = 1,
                    Size = UDim2.new(1, 0, 0, 20),
                    Text = "Good Afternoon, @" .. LocalPlayer.Name .. "!",
                    TextSize = 13,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    Parent = UserInfoContainer,
                })
        ...
        -- A viewport isolates tab transitions from the panel's reserved space.
        TabViewport = New("Frame", {
            BackgroundTransparency = 1,
            ClipsDescendants = true,
            Position = UDim2.fromOffset(0, UserPanelHeight),
            Size = UDim2.new(1, 0, 1, -UserPanelHeight),
            Parent = Container,
            ZIndex = 1,
        })
    end
